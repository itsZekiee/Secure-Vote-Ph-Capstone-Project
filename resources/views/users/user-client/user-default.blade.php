...
<main class="relative flex flex-col lg:flex-row items-center justify-center min-h-[90vh] py-12 px-4">
    <!-- Form Section -->
    <div class="w-full max-w-md animate-slide-up glass-effect rounded-2xl shadow-2xl p-8 border border-white/20 lg:mr-16"
         x-data="{
            showSignUp: false,
            isLoading: false,
            showPassword: false,
        ...
        <div class="text-center mb-8">
            <h1 class="text-3xl font-extrabold text-gray-900 mb-2" x-text="showSignUp ? 'Create Account' : 'Welcome Back'"></h1>
            <p class="text-gray-600 text-base" x-text="showSignUp ? 'Join our secure voting platform' : 'Sign in to access your voting form'"></p>
        </div>
        ...
        <div x-show="!showSignUp" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 transform scale-95" x-transition:enter-end="opacity-100 transform scale-100">
            <form class="space-y-5" @submit.prevent="isLoading = true">
                ...
                <button type="submit"
                        class="w-full bg-gradient-to-r from-[#1B1B1B] to-[#003153] text-white py-3 rounded-xl font-semibold hover:from-[#232323] hover:to-[#004080] focus:ring-4 focus:ring-blue-200 transition-all duration-200 transform hover:scale-[1.02]"
    ...
    <div class="hidden lg:block flex-1 max-w-2xl bg-transparent">
        <img src="{{ asset('asset/signUp.png') }}" alt="Login Illustration" class="w-full h-auto object-cover animate-fade-in" />
    </div>
</main>
</body>
</html>
